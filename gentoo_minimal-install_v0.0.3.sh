...
pkgUse ()
	{
    	echo "Setting package useflags..."
      		...
      		if [ ${iMAC} != "selinux" ]
      			then
      				command echo "chroot dbusproxy file-transfer globalcfg network userns apparmor contrib"
      			fi
      		if [ ${iPROFILE[0]} != "minimal" ]
      			then
      				command sed -i "s/USE=\"/USE=\"bluetooth ffmpeg extra ieee1394 v4l audit\ /" /etc/portage/make.conf
      				if [ ${iPROFILE[0]} == "desktop" ]
      					then
      						command sed -i "s/USE=\"/USE=\"wayland egl gnome gtk accessibility policykit cups opengl vaapi vulkan zstd gles2 X\ /" /etc/portage/make.conf
...
installMAC ()
	{
		if [ ${iMAC} == "selinux" ]
			then
				...
				command ${sEMERGE} sys-apps/apparmor sys-apps/apparmor-utils sys-libs/libapparmor sys-apps/firejail
			fi
	}
...
installBasedevelpkgs ()
  	{
    	...
    	command libtool --finish /usr/lib64
    	command ${sEMERGE} sys-process/audit app-misc/neofetch sys-fs/dosfstools sys-fs/cryptsetup net-misc/networkmanager net-dialup/rp-pppoe net-misc/modemmanager net-wireless/wpa_supplicant net-dns/dnsmasq app-admin/${iPRIVILEGE} net-misc/openssh sys-process/btop net-firewall/firewalld
    	command libtool --finish /usr/lib64
  	}
...
systemBoot ()
	{
    	echo "Installing & configuring system bootmethod..."
    		command echo "add_dracutmodules+=\" crypt crypt-gpg rootfs-block base dm btrfs systemd-ask-password \"" >> /etc/dracut.conf
        	command echo "uefi=\"yes\"" >> /etc/dracut.conf
          	...
          	if [ ${iLUKS} == "mapper/luksdev" ]
            	then
					...
					if [ ${sLUKS[2]} == true ] #checking if gpg-encrypted-keyfile is wanted
						then
							command sed -i "kernel_cmdline+=/$/\"/rd.luks.key=/crypt_key.luks.gpg:UUID=$(blkid -o value -s UUID /dev/${iDEVICE}1) \""
					else
...
userSet ()
  	{
    	echo "Setting up user account(s)..."
    		...
    		if [ ${iPRIVILEGE} == "doas" ]
    			then
    				command touch /etc/doas.conf
    				command chown -c root:root /etc/doas.conf
    				command chmod -c 0400 /etc/doas.conf
    				command echo "permit persist :wheel" > /etc/doas.conf
    		else
...
aliasLine ()
  	{
    	sSTARTA="neofetch && ${iPRIVILEGE} emaint -a sync && ${iPRIVILEGE} emerge -uDN @world && ${iPRIVILEGE} etc-update --automode -5 && ${iPRIVILEGE} emerge --depclean && ${iPRIVILEGE} emerge --oneshot sys-apps/portage"
    	if [ ${iNIXPKGmgr} == true ]
      		then
        		sNIXA=" && ${iPRIVILEGE} nix-env --upgrade"
      		fi
    	if [ ${iPROFILE[0]} == "desktop" ]
      		then
        		sFLATPAKA=" && flatpak update -y"
      		fi
        		sINITRAMFSA="&& ${iPRIVILEGE} dracut --regenerate-all --uefi"
...
enableServices ()
  	{
    	command systemctl enable firewalld systemd-timesyncd auditd NetworkManager systemd-boot-update
    	if [ ${iMAC} != "selinux" ]
    		then
        		...
        		if [ ${iPROFILE[0]} == "desktop" ]
          			then
            			command systemctl enable gdm
          				...
          				command systemctl enable docker sshd
          			fi
      		fi
...
if [ ${iPROFILE[0]} != "minimal" ]
	then
		. ./gentoo_${iPROFILE[0]}-install_v${sSCRIPT_VERSION}.sh
else
    		aliasLine
	    	writeBashrc
	    	enableServices
