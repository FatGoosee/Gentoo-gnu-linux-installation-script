...
createFilesystem ()
	{
	    echo "Creating filesystem..."
			...
			if [ ${sLUKS[0]} == true ]	#checking if encryption is wanted
				then
					iLUKS="mapper/luksdev"
					if [ ${sLUKS[2]} == true ]	#checking if gpg-encrypted-keyfile is wanted
						then
							command printf "\n\n\ny\ncrypt_key\ncrypt_key@encrypt.system\n\nO\n${sLUKS[1]}\n${sLUKS[1]}\n" | gpg --full-generate-key
							command printf "${sLUKS[1]}" | gpg --export-secret-keys crypt_key --output crypt_key.luks.gpg --encrypt
							sGPG="$(printf \"${sLUKS[1]}\" | gpg --decrypt crypt_key.luks.gpg)" 
							command printf "${sGPG}" | cryptsetup luksFormat -q -c aes-xts-plain64 -s 512 /dev/${iDEVICE}2 -
							command printf "${sGPG}\n${sLUKS[1]}\n" | cryptsetup luksAddKey -q /dev/${iDEVICE}2
					else
      		...
      		if [ ${sLUKS[2]} == true ]
		    	then
		    		command cp crypt_key.luks.gpg /mnt/gentoo/boot
		    	fi
	}
...
setupSystem ()
	{
		...
		command cp ${sMOUNTPOINT}/gentoo_${iPROFILE[0]}-install_v${sSCRIPT_VERSION}.sh /mnt/gentoo
